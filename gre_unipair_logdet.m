...
% % %Set up the movie.
% writerObj = VideoWriter('gre_unique_logdet.avi'); % Name it.
% writerObj.FrameRate = 5; % How many frames per second.
% open(writerObj); 
for k = 1: max_step
    % first obtain the assignment
    [r_pair_t_index{k}] = assign_gre_unipair_logdet_fun(M,pr,pt_hat{k}); 
    % use log_determinant
    %[~, r_pair_t_index{k}] = uni_pair_selection_fun(M,pr,pt_hat{k});
    ...
    for t = 1:M % for each target, update the estimate
       ...
       %  log_det_store(t,k) = log_det_singletar(pr([s1_index, s2_index], :), pt_hat{k}(t,:));         
