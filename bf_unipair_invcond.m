...
% % %Set up the movie.
% writerObj = VideoWriter('gre_unique_invcond.avi'); % Name it.
% writerObj.FrameRate = 5; % How many frames per second.
% open(writerObj); 
for k = 1: max_step
    % first obtain the assignment by brute-force unipair invercond
    [~, r_pair_t_index] = assign_bf_unipair_invcond_fun(N,M,pr,pt_hat{k});  
