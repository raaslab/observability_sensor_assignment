...
% log_det for three targets
%log_det_store = zeros(M, max_step); 
%log_det_sum = zeros(1,max_step);

%trace
trace_store = zeros(M, max_step); 
trace_sum = zeros(1,max_step);

% % % % %Set up the movie.
% writerObj = VideoWriter('gre_general_trace.avi'); % Name it.
% writerObj.FrameRate = 5; % How many frames per second.
% open(writerObj); 

for k = 1: max_step
    
    [tar_senset_inx,~] = assign_gre_gen_trace_fun(N,M,pr,pt_hat{k}); %pick the sensor set for each target as 1:M
     ...
     for t = 1:M % for each target 
       % find its sensor set
        ...
        % calculate the log_det for each target and its sensor pair            
        %log_det_store(t,k) = log_det_singletar(pr(t_sen_set, :), pt_hat{k}(t,:)); 
        %trace
        trace_store(t,k) = trace_nsensor_1tar(pr(t_sen_set, :), pt_hat{k}(t,:));
       ...
       % for each target and sensor-pair, plot 
           figure(1);
           title('Greedy General Assignment with Trace','fontsize',14)
