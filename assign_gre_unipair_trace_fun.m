...
function [r_pair_t_index] = assign_gre_unipair_trace_fun(M,pr,pt)

    pr_store = pr; % store the original sensor
    pt_store = pt; % store the original target

    %calculate the trace for N sensors and M targets to a matrix [N*N, M]
    ...
    for t=1:M
        ...
        %calculate the new trace table [(n-2)(n-2), m-1]. 
        trace_nn_m=trace_calcu_nn_m(pr,pt,length(pr(:,1)),length(pt(:,1))); 
