%compare the perfect pair assignment and unique pair assignment with trace
% this function describe how to make perfect matching and unique pair assignment
function [perfectvalue, unique_value, r_pair_t_index] = compare_relax_uni_trace_fun(M,pr,pt)

pr_store = pr; % store the original sensor
pt_store = pt; % store the original target

%using trace
trace_nn_m=trace_calcu_nn_m(pr,pt,length(pr(:,1)),length(pt(:,1)));

% perfect assignment
[perfectvalue, ~, ~]=bipartite_matching(trace_nn_m);
...
for t=1:M
    unique_eachstep(t)=max(max(trace_nn_m));
    [row_max,col_max]=find(trace_nn_m==unique_eachstep(t)); % how to make it unique??
